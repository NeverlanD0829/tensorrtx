...
int main(int argc, char** argv) {
    ...
    // 创建VideoWriter对象
    cv::VideoWriter video_writer("output.mp4", cv::VideoWriter::fourcc('M', 'P', '4', 'V'), fps, cv::Size(frame_width, frame_height));
    ...
    while (true) {
        ...
        // 将处理后的帧写入视频文件
        video_writer.write(img_batch[0]);
    ...
    context->destroy();  // 虽然有弃用警告，但仍然可以使用
    engine->destroy();
    runtime->destroy();
